...
LastTimeObserved<-read_feather("Load Data\\OPALCO\\OPALCO.feather")$time%>%min%>%as_datetime()%>%with_tz%>%as.Date-1

Times<-seq(LastTimeObserved,
           "2007-07-01"%>%as.Date,by=-1)%>%
...
data<-read_feather("Load Data\\OPALCO\\OPALCO.feather")
for(i in Times){
  print(i)
...
data%>%write_feather("Load Data\\OPALCO\\OPALCO.feather") 