...
LastTimeObserved<-read_feather("Load Data\\HG&E\\HGEWeather.feather")$time%>%min%>%as_datetime()%>%with_tz%>%as.Date-1
...
data<-read_feather("Load Data\\HG&E\\HGEWeather.feather")
for(i in Times){
  print(i)
...
data%>%write_feather("Load Data\\HG&E\\HGEWeather.feather") 