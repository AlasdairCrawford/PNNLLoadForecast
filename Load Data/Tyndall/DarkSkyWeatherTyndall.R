...
LastTimeObserved<-read_feather("Load Data\\Tyndall\\TyndallWeather.feather")$time%>%min%>%as_datetime()%>%with_tz%>%as.Date-1

Times<-seq(LastTimeObserved,
           "2017-10-01"%>%as.Date,by=-1)%>%
...
data<-read_feather("Load Data\\Tyndall\\TyndallWeather.feather")
for(i in Times){
  print(i)
...
data%>%write_feather("Load Data\\Tyndall\\TyndallWeather.feather") 