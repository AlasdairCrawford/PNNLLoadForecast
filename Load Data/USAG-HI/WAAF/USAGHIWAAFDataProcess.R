...
WAAFLoad<-read.csv("Load Data//USAG-HI//WAAF//WAAF.csv", header=T,stringsAsFactors = FALSE)%>%
...
WAAFLoad$Temp<- approx(USAGHIWeather$Time,USAGHIWeather$temperature,WAAFLoad$DateTime)$y
...
write_feather(WAAFLoad, "Load Data\\ProcessedData\\WAAF.feather")
